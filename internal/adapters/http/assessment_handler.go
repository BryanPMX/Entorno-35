...
func (h *AssessmentHandler) SendAssessmentEmail(c *gin.Context) {
	...
	err = h.assessmentService.SendAssessmentEmail(assessmentID, companyID)
	if err != nil {
		errMsg := err.Error()
		if strings.Contains(errMsg, "not found") {
			c.JSON(http.StatusNotFound, gin.H{"error": "Evaluacion no encontrada"})
		...
		if strings.Contains(errMsg, "does not have an email") {
			c.JSON(http.StatusBadRequest, gin.H{"error": "El empleado no tiene un correo electronico configurado"})
			return
		}
		c.JSON(http.StatusInternalServerError, gin.H{"error": errMsg})
		return
